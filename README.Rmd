...
`detectduplicate` will not report sequences of identical values as duplicates as `cor(rep(1, 3), rep(2, 3))` returns NA.
...
This risk is especially high if the data are reported with low precision, cardinality is low, or test are made for pairs of sequences with a constant multiplier and offset 
