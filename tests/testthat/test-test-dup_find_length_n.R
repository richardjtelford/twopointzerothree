...
test_that("works with no duplicates - identical", {
  ...
  expect_equal(colnames(f1), c("duplicate_no", "type", "length", "pos1", "vec1", "pos2", "vec2", "delta", "n_distinct"))
})
...
test_that("works with no duplicates - multiply-offset", {
  ...
  expect_s3_class(f1, "data.frame")
  expect_equal(nrow(f1), 0)
  expect_equal(colnames(f1), c("duplicate_no", "type", "length", "pos1", "vec1", "pos2", "vec2", "delta", "offset", "multiply", "n_distinct"))
})
